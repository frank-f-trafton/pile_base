...
Creates string parsing objects.
...
A *walker* object ties Lua search methods to an internal position index. Generally, when a search is successful, the position advances past the match region, and when unsuccessful, it stays put (or throws an error.)

Some knowledge of Lua's `string` module is necessary to understand what is going on here. Refer to these sections of the Lua 5.1 Reference Manual: [5.4 - String Manipulation](https://www.lua.org/manual/5.1/manual.html#5.4) and its subsection [5.4.1 - Patterns](https://www.lua.org/manual/5.1/manual.html#5.4.1).
...
* `[s]`: An optional string to assign. If not provided, the walker will start out with an empty string.

* `[name]`: An optional name to use when generating warnings and error messages.
...
`local ln, cn  = function pStringWalk.countLineChar(s, i, j, ln, cn)`
...
* The input string is expected to be valid UTF-8 text.

* This function can be used instead of `W:getLineCharNumbers()` if you are collecting sequential line and character numbers in a loop. While the walker method begins counting from the first byte every time, this function can start from any valid UTF-8 start byte.
...
Assigns or clears the walker name, which is used when generating warnings and error messages.
...
* The name is not cleared by `W:setString()` or `W:reset()`. Nor is it used when Terse Mode is enabled.
...
Like `W:find()`, but the search is conducted with `string.find()`'s *plain mode*. All pattern-matching magic characters are treated as plain characters. As such, this method does not support captures.
...
* The search is anchored to the walker's position. To search the remainder of the string for a literal substring, use `W:plain[Req]()`.
...
Like W:match(), but raises a Lua error when the search is unsuccessful.
...
Like W:bytes(), but raises a Lua error when the request is unsuccessful.
...
Raises a Lua error. Depending on the walker's configuration, the output may include a name, a line number, and a character number. If the walker is in Terse Mode, then a basic *parsing failed* message will be displayed instead.
...
Prints a warning message. Depending on the walker's configuration, the output may include a name, a line number, and a character number. If the walker is in Terse Mode, no message is printed.
...
Pushes a new string onto the walker, moving the existing string and position onto a stack. The walker's position becomes 1.
...
That said, it's possible to match single code points with a pattern defined in Lua 5.3: [utf8.charpattern](https://www.lua.org/manual/5.3/manual.html#pdf-utf8.charpattern). PILE StringWalk contains a modified version of this pattern, stored in `pStringWalk.ptn_code`.
