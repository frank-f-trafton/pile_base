...
-- [===[
self:registerFunction("pStringWalk.new()", pStringWalk.new)

self:registerJob("pStringWalk.new()", function(self)
	self:expectLuaError("arg #1 bad type", pStringWalk.new, {})
	self:expectLuaReturn("arg #1 nil is acceptable", pStringWalk.new, nil)
	...
	do
		...
		self:isEqual(W.S, "foobar")
		self:isEqual(W.I, 1)
		self:isEqual(W._name, "my_walker")
		self:lf(4)
	end
end
)
...
-- [===[
self:registerFunction("W:setString()", _mt_walk.setString)

self:registerJob("W:setString()", function(self)
	do
		local W = pStringWalk.new()
		self:expectLuaError("arg #1 bad type", _mt_walk.setString, W, nil)
	end
...
-- [===[
self:registerFunction("W:setName()", _mt_walk.setName)

self:registerJob("W:setName()", function(self)
	do
		local W = pStringWalk.new()
		self:expectLuaError("arg #1 bad type", _mt_walk.setName, W, true)
	end

	do
		self:print(3, "[+] set name")
		local W = pStringWalk.new("foobar")
		W:setName("mojo")
...
-- [===[
self:registerFunction("W:getName()", _mt_walk.getName)

self:registerJob("W:getName()", function(self)
	do
		self:print(3, "[+] get name")
		local W = pStringWalk.new("foobar")
		W:setName("bozo")
...
-- [===[
self:registerFunction("W:getTerseMode()", _mt_walk.getTerseMode)

self:registerJob("W:getTerseMode()", function(self)
	do
		self:print(3, "[+] expected behavior")
		local W = pStringWalk.new("foobar")
		W:setTerseMode(true)
...
-- [===[
self:registerFunction("W:getByteMode()", _mt_walk.getByteMode)

self:registerJob("W:getByteMode()", function(self)
	do
		self:print(3, "[+] expected behavior")
		local W = pStringWalk.new("foobar")
		W:setByteMode(true)
...
-- [===[
self:registerFunction("W:getLineCharDisplay()", _mt_walk.getLineCharDisplay)

self:registerJob("W:getLineCharDisplay()", function(self)
	do
		self:print(3, "[+] expected behavior")
		local W = pStringWalk.new("foobar")
		W:setLineCharDisplay(true, true)
...
self:registerJob("W:find()", function(self)
	...
	do
		...
		--]]
		local W = pStringWalk.new("abcdefghijklmnopqrstuvwxyz")
...
self:registerJob("W:plain()", function(self)
	do
		local W = pStringWalk.new("fo(oba)r") -- literal, not a capture
...
self:registerJob("W:ws()", function(self)
	do
		...
		local W = pStringWalk.new("foo   bar")
		W:step(3)
		W:ws()
		self:isEqual(W.I, 7)
		self:lf(4)
...
self:registerJob("W:wsNext()", function(self)
	do
		...
		local W = pStringWalk.new("   foooooo   ")
		print("W.I", W.I)
		W:step(3)
		print("W.I", W.I)
		W:wsNext()
...
self:registerJob("W:getLineCharNumbers()", function(self)
	...
	do
		...
		local W = pStringWalk.new("abcdefg")
		local ln, cn
...
-- [===[
self:registerFunction("pStringWalk.countLineChar()", pStringWalk.countLineChar)

self:registerJob("pStringWalk.countLineChar()", function(self)
...
dog]]
		...
		for i, word in str:gmatch("()([a-z]+)") do
			ln, cn = pStringWalk.countLineChar(str, i, j, ln, cn)
			j = i
...
self:registerJob("W:error()", function(self)
	...
	do
		...
		W:setTerseMode(true)
		W:setName("This name should not appear.")
	...
	do
		...
		W:setName("very_important_document.txt")
		local ok, err = pcall(_mt_walk.error, W, "Test error.")
		self:print(4, err)
		self:isEvalTrue(err:find("very_important_document.txt", 1, true))
		self:lf(4)
	end
end
)
...
self:registerJob("W:warn()", function(self)
	do
		...
		W:warn(":^)")
		W:setTerseMode(false)
		W:warn("This warning should have the name 'Rodney', a line number, and a character number. It should *not* be preceded by an ASCII smiley.")
		self:lf(4)
	end
...
-- [===[
self:registerJob("pStringWalk.ptn_code", function(self)
	do
		local W = pStringWalk.new("abcdë§偕𐀀")
		self:isEqual(W:match(pStringWalk.ptn_code), "a")
		self:isEqual(W:match(pStringWalk.ptn_code), "b")
		self:isEqual(W:match(pStringWalk.ptn_code), "c")
		self:isEqual(W:match(pStringWalk.ptn_code), "d")
		self:isEqual(W:match(pStringWalk.ptn_code), "ë")
		self:isEqual(W:match(pStringWalk.ptn_code), "§")
		self:isEqual(W:match(pStringWalk.ptn_code), "偕")
		self:isEqual(W:match(pStringWalk.ptn_code), "𐀀")
		self:isEqual(W:match(pStringWalk.ptn_code), nil)
	end
end
)
