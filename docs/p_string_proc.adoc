...
Dependencies: xref:p_assert.adoc[pAssert], xref:p_interp.adoc[pInterp], xref:p_string_walk.adoc[pStringWalk]
...
* `symbols`: The xref:p_string_proc.adoc#_symbols[symbol] lookup table. All keys must be strings, and all values must be one of: `string`, `number`, `function`, or boolean `true`.
...
Checks xref:p_string_proc.adoc#_word[words] in a production table against the keys in a symbol lookup table. Raises an error if the symbol table does not have entries for all words in the production table.
...
* `symbols`: The xref:p_string_proc.adoc#_symbols[symbol] lookup table.
...
**Returns:** The converted table, to be used with xref:p_string_proc.adoc#_pstringproc_traverse[pStringProc.traverse].
...
* `W`: The xref:p_string_walk.adoc#_walkers[walker] object, configured with the string to be processed and its start position.

* `t`: A production table for the string, having been created by xref:p_string_proc.adoc#_pstringproc_totable[pStringProc.toTable].

* `symbols`: A lookup table of xref:p_string_proc.adoc#_symbols[symbols].
...
On success, xref:p_string_proc.adoc#_pstringproc_traverse[pStringProc.traverse] returns a table of values that were fetched while processing the string:
...
** Takes `W` (the xref:p_string_walk.adoc#_walkers[walker]) as its only argument.
