...
A wrapper for xref:p_table.adoc#_ptable_resolve[pTable.resolve] which raises an error if no value was found.
...
Like xref:p_table.adoc#_ptable_isarrayonly[pTable.isArrayOnly], but carves out an exception for index zero.
...
Creates a xref:p_table.adoc#_namedmaps[NamedMap].
...
Creates a xref:p_table.adoc#_namedmaps[NamedMap]. Like xref:p_table.adoc#_ptable_newnamedmap[pTable.newNamedMap], but uses a vararg list to populate keys. All values in the returned table are `true`.
...
Assigns a xref:p_table.adoc#_double_variables[double variable], falling back to a default value when `false`/`nil` is provided.
...
Updates a xref:p_table.adoc#_double_variables[double variable]'s internal value, if the user-facing value is `false`/`nil`.
...
A NamedMap's name can be retrieved with xref:p_name.adoc#_pname_safeget[pName.safeGet]. For example:
