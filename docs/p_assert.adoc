...
Dependencies: xref:p_interp.adoc[pInterp], xref:p_name.adoc[pName]
...
The value is in a xref:p_table.adoc#_namedmaps[NamedMap] (or any other Lua table) as a key.
...
The value is `false`/`nil`, or it appears in a xref:p_table.adoc#_namedmaps[NamedMap] (or any other Lua table) as a key.
...
The name tag can be used in a few different ways (see xref:p_assert.adoc#_uses_of_name_tags[Uses of Name Tags]).
