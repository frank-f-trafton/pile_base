...
Like <<_ptree_nodegetindex>>, but assumes the caller has already checked the parent and obtained a reference to the table of siblings. **Cannot be used with root nodes.**
...
Gets the node's parent. Like <<_ptree_nodegetparent>>, but raises an error if the parent value is not a table. **Cannot be used with root nodes.**
