...
Creates a new <<_model,model>> with a <<_keys,'keys'>> filter and the <<_model_options,model option>> `reject_unhandled` enabled. (This is a common configuration in the workloads that __pSchema__ was designed for.)
...
* `t`: The table to use as the <<_model,model>>. Should be a freshly created table with no metatable.
...
A table that describes how one kind of table should be validated. Has <<_filter,filters>> which associate keys with <<_handler,handler>> functions and other models.


==== Handler

A function that validates one value in a table. handlers are based on the function signature used in <<p_assert.adoc#_the_passert_function_signature,pAssert>>. On failure, handlers should raise an error; on success, they should do nothing.
...
An instance of a <<_handler,handler>> or a <<_model,model>>.
