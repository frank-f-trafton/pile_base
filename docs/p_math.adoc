...
This function skips xref:p_math.adoc#_pmath_dist[pMath.dist]'s call to https://www.lua.org/manual/5.1/manual.html#pdf-math.sqrt[**math.sqrt**]. It may be faster when comparing the distances of two sets of points.
...
See xref:p_math.adoc#_rounding_error[Module Notes: Rounding Error]
...
Rounds a number toward infinity. Like xref:p_math.adoc#_pmath_roundinf[pMath.roundInf], but with support for increments other than 1.
...
xref:p_math.adoc#_pmath_roundinf[pMath.roundInf] and xref:p_math.adoc#_pmath_roundinfincrement[pMath.roundInfIncrement] are susceptible to floating point rounding error when the input is extremely close to, but not exactly 0.5 or -0.5:
