...
* Shortened the module name `p_rectangle.lua` to `p_rect.lua`.

* `p_line.lua`:

  * Renamed the `SideDelta` structure to `LineDelta`, to avoid confusion with `p_rect.lua`'s own `SideDelta`.

* `p_table.lua`:

  * Fixed a bad variable name in `pTable.set()` and added missing tests for this function.

  * Renamed some functions:

    * `pTable.arrayOfHashKeys()` -> `pTable.makeArrayFromKeys()`

    * `pTable.invertLUT()` -> `pTable.invertKeysAndValues()`

    * `pTable.removeElement()` -> `pTable.removeValueFromArray()`

    * `pTable.valueInArray()` -> `pTable.isValueInArray()`

* `p_utf8_conv.lua`:

  * Renamed all functions:

    * `pUtf8Conv.latin1_utf8()` -> `pUtf8Conv.fromLatin1ToUtf8()`

    * `pUtf8Conv.utf16_utf8()` -> `pUtf8Conv.fromUtf16ToUtf8()`

    * `pUtf8Conv.utf8_latin1()` -> `pUtf8Conv.fromUtf8ToLatin1()`

    * `pUtf8Conv.utf8_utf16()` -> `pUtf8Conv.fromUtf8ToUtf16()`

* In camel cased function/method names, changed the casing of acronyms:

  * `UTF8` -> `Utf8`

  * `pAssert.integerGE()` -> `pAssert.integerGe()`

  * `pAssert.integerGEEval()` -> `pAssert.integerGeEval()`

  * `pAssert.notNaN()` -> `pAssert.notNan()`

  * `pAssert.notNilNotNaN()` -> `pAssert.notNilNotNan()`

  * `pAssert.notNilNotFalseNotNaN()` -> `pAssert.notNilNotFalseNotNan()`

  * `pAssert.numberGE()` -> `pAssert.numberGe()`

  * `pAssert.numberGEEval()` -> `pAssert.numberGeEval()`

  * `pAssert.numberNotNaN()` -> `pAssert.numberNotNan()`

  * `pAssert.numberNotNaNEval()` -> `pAssert.numberNotNanEval()`

  * `pTable.newLUT()` -> `pAssert.newLut()`

  * `pTable.newLUTV()` -> `pAssert.newLutV()`

* Converted the documentation from Markdown to AsciiDoc, and reorganized the Git repo.

* This changelog: Crossed out an incorrect comment about enums, and replaced tabs with spaces.
